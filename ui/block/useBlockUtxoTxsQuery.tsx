...
import type { BlockTransactionsResponse, BlockUtxoTransactionsResponse } from 'types/api/block';
...
export type BlockUtxoTxsQuery = QueryWithPagesResult<'block_utxo_txs'> & {
...
export default function useBlockUtxoTxsQuery({ heightOrHash, blockQuery, tab }: Params): BlockUtxoTxsQuery {
  const [ isRefetchEnabled, setRefetchEnabled ] = React.useState(false);

  const apiQuery = useQueryWithPages({
    resourceName: 'block_utxo_txs',
    pathParams: { height_or_hash: heightOrHash },
    options: {
      enabled: Boolean(tab === 'utxo_txs' && !blockQuery.isPlaceholderData && !blockQuery.isDegradedData),
      placeholderData: generateListStub<'block_utxo_txs'>(UTXO_TX, 50, { next_page_params: {
  ...
  const rpcQuery = useQuery<RpcResponseType, unknown, BlockTransactionsResponse | null>({
    queryKey: [ 'RPC', 'block_utxo_txs', { heightOrHash } ],
    queryFn: async() => {
      if (!publicClient) {
        return null;
      }
    ...
    enabled: publicClient !== undefined && tab === 'utxo_txs' && (blockQuery.isDegradedData || apiQuery.isError || apiQuery.errorUpdateCount > 0),
    retry: false,
    refetchOnMount: false,
  });
  ...
  const rpcQueryWithPages: QueryWithPagesResult<'block_utxo_txs'> = React.useMemo(() => {
    return {
      ...rpcQuery as UseQueryResult<BlockUtxoTransactionsResponse, ResourceError>,
