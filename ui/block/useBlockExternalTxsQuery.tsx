...
export default function useBlockExternalTxsQuery({ heightOrHash, blockQuery, tab }: Params): BlockEtxQuery {
  const [ isRefetchEnabled, setRefetchEnabled ] = React.useState(false);

  const apiQuery = useQueryWithPages({
    resourceName: 'block_txs',
    pathParams: { height_or_hash: heightOrHash },
    filters: { ext_type: 'external' },
    options: {
      enabled: Boolean(tab === 'external' && !blockQuery.isPlaceholderData),
