...
export type BlockExtTxsQuery = QueryWithPagesResult<'block_ext_txs'> & {
...
export default function useBlockExtTxsQuery({ heightOrHash, blockQuery, tab }: Params): BlockExtTxsQuery {
  const [ isRefetchEnabled, setRefetchEnabled ] = React.useState(false);

  const apiQuery = useQueryWithPages({
    resourceName: 'block_ext_txs',
    pathParams: { height_or_hash: heightOrHash },
    options: {
      enabled: Boolean(tab === 'ext_txs' && !blockQuery.isPlaceholderData && !blockQuery.isDegradedData),
      placeholderData: generateListStub<'block_ext_txs'>(TX, 50, { next_page_params: {
  ...
  const rpcQuery = useQuery<RpcResponseType, unknown, BlockTransactionsResponse | null>({
    queryKey: [ 'RPC', 'block_ext_txs', { heightOrHash } ],
    queryFn: async() => {
      if (!publicClient) {
        return null;
      }
    ...
    enabled: publicClient !== undefined && tab === 'ext_txs' && (blockQuery.isDegradedData || apiQuery.isError || apiQuery.errorUpdateCount > 0),
    retry: false,
    refetchOnMount: false,
  });
  ...
  const rpcQueryWithPages: QueryWithPagesResult<'block_ext_txs'> = React.useMemo(() => {
    return {
